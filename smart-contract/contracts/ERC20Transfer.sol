...
interface ZetaGiftInterface {
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool);
    function approve(address spender, uint256 amount) external returns (bool);
    function balanceOf(address account) external view returns (uint256);
}

contract ZetaGiftTransfer {
    ...
    modifier onlyOwner {
        require(msg.sender == owner, "ZetaGiftTransfer: caller is not the owner");
        _;
    }

    modifier onlyVerifiedToken(address _token) {
        require(verifiedTokens[_token] == true, "ZetaGiftTransfer: token is not verified");
        _;
    }
    ...
    function removeVerifiedToken(address _token) public onlyOwner {
        require(verifiedTokens[_token] == true, "ZetaGiftTransfer: token is not in the list");
    ...
    function transfer(ZetaGiftInterface token, address to, uint256 amount, string memory message) public onlyVerifiedToken(address(token)) returns (bool) {
        uint256 senderBalance = token.balanceOf(msg.sender);
        require(senderBalance >= amount, "ZetaGiftTransfer: insufficient balance");

        bool success = token.transferFrom(msg.sender, to, amount);
        require(success, "ZetaGiftTransfer: transfer failed");
